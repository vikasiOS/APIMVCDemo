...
class NFXGenericController: NFXViewController
{
    ...
    func formatNFXString(_ string: String) -> NSAttributedString {
        ...
        let matchesBodyHeaders = regexBodyHeaders.matches(in: string, options: NSRegularExpression.MatchingOptions.withoutAnchoringBounds, range: NSRange(location: 0, length: l)) as Array<NSTextCheckingResult>
        ...
        let matchesKeys = regexKeys.matches(in: string, options: NSRegularExpression.MatchingOptions.withoutAnchoringBounds, range: NSRange(location: 0, length: l)) as Array<NSTextCheckingResult>
