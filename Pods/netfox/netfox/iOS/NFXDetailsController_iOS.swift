...
class NFXDetailsController_iOS: NFXDetailsController, MFMailComposeViewControllerDelegate {
    ...
    @objc func actionButtonPressed(_ sender: UIBarButtonItem) {
        ...
        let simpleLog: UIAlertAction = UIAlertAction(title: "Simple log", style: .default) { [unowned self] _ -> Void in
            self.sendMailWithBodies(false)
        }
        actionSheetController.addAction(simpleLog)

        let fullLogAction: UIAlertAction = UIAlertAction(title: "Full log", style: .default) { [unowned self] _ -> Void in
