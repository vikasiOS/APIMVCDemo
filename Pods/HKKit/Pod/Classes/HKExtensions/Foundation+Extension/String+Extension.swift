...
extension String {
    ...
    public func hk_toRange() -> NSRange {
       ...
       return NSRange(location: self.distance(from: self.startIndex, to: range!.lowerBound), length: self.count)
    ...
    public func hk_isEmailValid() -> Bool {
        ...
        return regex?.firstMatch(in: self.hk_trimWhiteSpace()!, options: [], range: NSRange(location: 0, length: self.hk_trimWhiteSpace()!.count)) != nil
